import js from '@eslint/js'
import globals from 'globals'
import reactHooks from 'eslint-plugin-react-hooks'
import reactRefresh from 'eslint-plugin-react-refresh'
import tseslint from 'typescript-eslint'
import { defineConfig, globalIgnores } from 'eslint/config'

export default defineConfig([
  globalIgnores(['dist']),
  {
    files: ['**/*.{ts,tsx}'],
    extends: [
      js.configs.recommended,
      // Lint type-aware: habilita reglas como no-floating-promises (A14 —
      // ninguna promesa de API sin manejo de error) y no-unsafe-* (A11/A12 —
      // nada de `any` tapando los contratos del backend).
      tseslint.configs.recommendedTypeChecked,
      reactHooks.configs.flat.recommended,
      reactRefresh.configs.vite,
    ],
    languageOptions: {
      globals: globals.browser,
      parserOptions: {
        projectService: true,
        tsconfigRootDir: import.meta.dirname,
      },
    },
    rules: {
      // Permite prefijar con `_` los args/variables intencionalmente sin usar
      // (props destructuradas, callbacks de eventos) en vez de silenciarlos.
      '@typescript-eslint/no-unused-vars': [
        'error',
        { argsIgnorePattern: '^_', varsIgnorePattern: '^_' },
      ],
    },
  },
  {
    // Archivos de test/helpers de test: nunca pasan por Vite Fast Refresh,
    // así que exportar helpers no-componente (ej. `renderConProveedores` en
    // `src/test/utils.tsx`) o hacer `export *` de una librería (ej.
    // `@testing-library/react`) no rompe HMR. Se desactiva sólo acá — el
    // resto de componentes de producción sigue exigiendo la regla.
    files: ['src/test/**', '**/__tests__/**', '**/*.test.{ts,tsx}'],
    rules: {
      'react-refresh/only-export-components': 'off',
    },
  },
])
